...
### Barry's Ideas (do not lose these)
- [ ] **Ternary computing — `\|/` notation** — dreamed up a balanced ternary system where `\`=-1 (false), `|`=0 (uncertain), `/`=+1 (true). Symbols represent geometric orientation, not just magnitude. Potential applications: (1) build a `\|/` ternary interpreter/arithmetic library, (2) wire ternary belief states into SOMA's cognition so she has genuine uncertainty rather than binary true/false, (3) write it up — the visual notation is novel and not in the literature. This is real math (balanced ternary, Soviet Setun 1958, Knuth) with a new representation layer on top.
